...
namespace EyouSoft.BLL.AdminCenterStructure
{
    ...
    public class AttendanceInfo : EyouSoft.BLL.BLLBase
    {
        ...
        /// <param name="Month">月份(为0时查全年)</param>
        ...
        /// <param name="CompanyId">公司编号</param>
        /// <returns></returns>
        public IList<EyouSoft.Model.AdminCenterStructure.AttendanceAbout> GetList(int PageSize, int PageIndex, ref int RecordCount, string ArchiveNo, string StaffName, int DepartmentId,string deptNm, int CompanyId)
        {
            return idal.GetList(PageSize, PageIndex, ref RecordCount, ArchiveNo, StaffName, DepartmentId,deptNm, CompanyId);
        }
        /// <summary>
        /// 按考勤时间获取单个员工考勤信息
        /// </summary>
        ...
        public bool Add(IList<EyouSoft.Model.AdminCenterStructure.AttendanceInfo> lists)
        {
            ...
            if (IsTrue)
            {
                ...
                logInfo.EventMessage = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在" + Model.EnumType.PrivsStructure.Privs2.行政中心_考勤管理.ToString() + "新增了考勤管理数据。";
                logInfo.EventTime = DateTime.Now;
                logInfo.EventTitle = "新增考勤";
                logInfo.ModuleId = Model.EnumType.PrivsStructure.Privs2.行政中心_考勤管理;
        ...
        public bool Update(int CompanyId, IList<EyouSoft.Model.AdminCenterStructure.AttendanceInfo> lists, DateTime AddDate)
        {
            ...
            if (IsTrue)
            {
                ...
                logInfo.EventCode = EyouSoft.Model.CompanyStructure.SysHandleLogsNO.EventCode;
                logInfo.EventIp = string.Empty;
                logInfo.EventMessage = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在" + Model.EnumType.PrivsStructure.Privs2.行政中心_考勤管理.ToString() + "修改了考勤管理数据。";
                logInfo.EventTime = DateTime.Now;
        ...
        public bool Delete(int CompanyId, string Id)
        {
            ...
            if (IsTrue)
            {
                ...
                logInfo.EventCode = EyouSoft.Model.CompanyStructure.SysHandleLogsNO.EventCode;
                logInfo.EventIp = string.Empty;
                logInfo.EventMessage = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在" + Model.EnumType.PrivsStructure.Privs2.行政中心_考勤管理.ToString() + "删除了考勤管理数据，编号为：" + Id;
                logInfo.EventTime = DateTime.Now;
