    ...
    public class DutyManager : EyouSoft.BLL.BLLBase
    {
        ...
        public int Add(EyouSoft.Model.AdminCenterStructure.DutyManager model)
        {
            ...
            if (RowCount > 0)
            {
                ...
                logInfo.EventMessage = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在" + Model.EnumType.PrivsStructure.Privs2.行政中心_职务管理.ToString() + "新增了职务信息数据。";
                logInfo.EventTime = DateTime.Now;
                logInfo.EventTitle = "新增职务";
                logInfo.ModuleId = Model.EnumType.PrivsStructure.Privs2.行政中心_职务管理;
        ...
        public int Update(EyouSoft.Model.AdminCenterStructure.DutyManager model)
        {
            ...
            if (RowCount > 0)
            {
                ...
                logInfo.EventCode = EyouSoft.Model.CompanyStructure.SysHandleLogsNO.EventCode;
                logInfo.EventIp = string.Empty;
                logInfo.EventMessage = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在" + Model.EnumType.PrivsStructure.Privs2.行政中心_职务管理.ToString() + "修改了职务信息数据，编号为：" + model.Id;
                logInfo.EventTime = DateTime.Now;
        ...
        public bool Delete(int CompanyId, int DutyId)
        {
            ...
            if (IsTrue)
            {
                ...
                logInfo.EventCode = EyouSoft.Model.CompanyStructure.SysHandleLogsNO.EventCode;
                logInfo.EventIp = string.Empty;
                logInfo.EventMessage = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在" + Model.EnumType.PrivsStructure.Privs2.行政中心_职务管理.ToString() + "删除了职务信息数据，编号为：" + DutyId;
                logInfo.EventTime = DateTime.Now;
