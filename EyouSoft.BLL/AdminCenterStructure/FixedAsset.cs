    ...
    public class FixedAsset : EyouSoft.BLL.BLLBase
    {
        ...
        public bool Add(EyouSoft.Model.AdminCenterStructure.FixedAsset model)
        {
            ...
            if (IsTrue)
            {
                ...
                logInfo.EventMessage = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在" + Model.EnumType.PrivsStructure.Privs2.行政中心_固定资产管理.ToString() + "新增了固定资产信息数据。";
                logInfo.EventTime = DateTime.Now;
                logInfo.EventTitle = "新增固定资产信息";
                logInfo.ModuleId = Model.EnumType.PrivsStructure.Privs2.行政中心_固定资产管理;
        ...
        public bool Update(EyouSoft.Model.AdminCenterStructure.FixedAsset model)
        {
            ...
            if (IsTrue)
            {
                ...
                logInfo.EventCode = EyouSoft.Model.CompanyStructure.SysHandleLogsNO.EventCode;
                logInfo.EventIp = string.Empty;
                logInfo.EventMessage = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在" + Model.EnumType.PrivsStructure.Privs2.行政中心_固定资产管理.ToString() + "修改了固定资产信息数据，编号为：" + model.Id;
                logInfo.EventTime = DateTime.Now;
        ...
        public bool Delete(int CompanyId, int Id)
        {
            ...
            if (IsTrue)
            {
                ...
                logInfo.EventCode = EyouSoft.Model.CompanyStructure.SysHandleLogsNO.EventCode;
                logInfo.EventIp = string.Empty;
                logInfo.EventMessage = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在" + Model.EnumType.PrivsStructure.Privs2.行政中心_固定资产管理.ToString() + "删除了固定资产信息数据，编号为：" + Id;
                logInfo.EventTime = DateTime.Now;
