    ...
    public class TrainPlan : EyouSoft.BLL.BLLBase
    {
        ...
        public bool Add(EyouSoft.Model.AdminCenterStructure.TrainPlan model)
        {
            ...
            if (IsTrue)
            {
                ...
                logInfo.EventMessage = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在" + Model.EnumType.PrivsStructure.Privs2.行政中心_培训计划.ToString() + "新增了培训计划信息数据。";
                logInfo.EventTime = DateTime.Now;
                logInfo.EventTitle = "新增培训计划信息";
                logInfo.ModuleId = Model.EnumType.PrivsStructure.Privs2.行政中心_培训计划;
        ...
        public bool Update(EyouSoft.Model.AdminCenterStructure.TrainPlan model)
        {
            ...
            if (IsTrue)
            {
                ...
                logInfo.EventCode = EyouSoft.Model.CompanyStructure.SysHandleLogsNO.EventCode;
                logInfo.EventIp = string.Empty;
                logInfo.EventMessage = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在" + Model.EnumType.PrivsStructure.Privs2.行政中心_培训计划.ToString() + "修改了培训计划信息数据，编号为：" + model.Id;
                logInfo.EventTime = DateTime.Now;
        ...
        public bool Delete(int CompanyId, int Id)
        {
            ...
            if (IsTrue)
            {
                ...
                logInfo.EventCode = EyouSoft.Model.CompanyStructure.SysHandleLogsNO.EventCode;
                logInfo.EventIp = string.Empty;
                logInfo.EventMessage = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在" + Model.EnumType.PrivsStructure.Privs2.行政中心_培训计划.ToString() + "删除了培训计划信息数据，编号为：" + Id;
                logInfo.EventTime = DateTime.Now;
