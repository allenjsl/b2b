...
namespace EyouSoft.BLL.CompanyStructure
{
    ...
    public class News : BLLBase
    {
        ...
        /// <returns></returns>
        public IList<Model.CompanyStructure.News> GetList(int pageSize, int pageIndex, ref int recordCount, int companyId,string zxsId)
        {
        ...
        /// <param name="zxsId">专线商编号</param>
        /// <returns></returns>
        public IList<Model.PersonalCenterStructure.NoticeNews> GetAcceptNews(int pageSize, int pageIndex, ref int recordCount, int userId
            , int companyId,string zxsId)
        {
            if (pageSize <= 0 || pageIndex <= 0 || userId <= 0 || companyId <= 0) return null;
            return this._dal.GetAcceptNews(pageSize, pageIndex, ref recordCount, userId, companyId,zxsId);
        }
        ...
        private Model.CompanyStructure.SysHandleLogs AddLogs(string actionName, string id, string name)
        {
            var model = new Model.CompanyStructure.SysHandleLogs
                {
                    ModuleId = Model.EnumType.PrivsStructure.Privs2.系统设置_信息管理,
                    EventCode = Model.CompanyStructure.SysHandleLogsNO.EventCode,
                    EventMessage =
                        DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在"
                        + Model.EnumType.PrivsStructure.Privs2.系统设置_信息管理 + actionName + "了信息管理数据，编号为" + id
                        + (string.IsNullOrEmpty(name) ? string.Empty : "，标题为" + name),
                    EventTitle = actionName + Model.EnumType.PrivsStructure.Privs2.系统设置_信息管理 + "数据"
                };
        ...
        {
            return this._dal.IsNews(companyId, departId, UserId,zxsId);
        }
    }
}
