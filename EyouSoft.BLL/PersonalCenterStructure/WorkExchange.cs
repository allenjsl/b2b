...
namespace EyouSoft.BLL.PersonalCenterStructure
{
    ...
    public class WorkExchange : EyouSoft.BLL.BLLBase
    {
            ...
            if (Result)
            {
                HandleLogsBll.Add(
                       new EyouSoft.Model.CompanyStructure.SysHandleLogs()
                       {
                           ...
                           EventMessage = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在" + Model.EnumType.PrivsStructure.Privs2.个人中心_工作交流.ToString() + "新增了工作交流！编号为：" + model.ExchangeId,
            ...
            if (Result)
            {
                HandleLogsBll.Add(
                       new EyouSoft.Model.CompanyStructure.SysHandleLogs()
                       {
                           ModuleId = EyouSoft.Model.EnumType.PrivsStructure.Privs2.个人中心_工作交流,
                           EventCode = Model.CompanyStructure.SysHandleLogsNO.EventCode,
                           EventMessage = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在" + Model.EnumType.PrivsStructure.Privs2.个人中心_工作交流.ToString() + "修改了工作交流！编号为：" + model.ExchangeId,
            ...
            if (Result)
            {
                HandleLogsBll.Add(
                       new EyouSoft.Model.CompanyStructure.SysHandleLogs()
                       {
                           ModuleId = EyouSoft.Model.EnumType.PrivsStructure.Privs2.个人中心_工作交流,
                           EventCode = Model.CompanyStructure.SysHandleLogsNO.EventCode,
                           EventMessage = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "{0}在" + Model.EnumType.PrivsStructure.Privs2.个人中心_工作交流.ToString() + "回复了工作交流！编号为：" + model.ExchangeId,
                           EventTitle = "回复" + Model.EnumType.PrivsStructure.Privs2.个人中心_工作交流.ToString() + "数据"
                       });
            }
            return Result;
        }
        ...
        public IList<EyouSoft.Model.PersonalCenterStructure.WorkExchange> GetList(
            ...
            int CompanyId,
            int OperatorId,
            EyouSoft.Model.PersonalCenterStructure.WorkExchangeSearch search)
        {

            return idal.GetList(pageSize, pageIndex, ref RecordCount, CompanyId, OperatorId, search);
        }
        ...
        /// <returns></returns>
        public IList<EyouSoft.Model.PersonalCenterStructure.WorkExchangeReply> GetList(int CompanyId, int ExchangeId, int pageSize, int pageIndex, ref int RecordCount)
        {
            return idal.GetList(CompanyId, ExchangeId,pageSize, pageIndex, ref RecordCount);
        }
