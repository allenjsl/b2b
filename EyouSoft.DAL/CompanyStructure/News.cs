...
namespace EyouSoft.DAL.CompanyStructure
{
    ...
    public class News : DALBase, IDAL.CompanyStructure.INews
    {
        ...
        public int Add(Model.CompanyStructure.News model)
        {
            ...
            strSql.Append(
                " INSERT INTO [tbl_News] ([CompanyId],[Title],[OperatorId],[IssueTime],[Content],[Views],[Files],[IsDelete],[ZxsId])  VALUES  (@CompanyId,@Title,@OperatorId,@IssueTime,@Content,@Views,@Files,@IsDelete,@ZxsId); ");
                            ...
                            new Model.CompanyStructure.NewsAccept
                                {
                                    ...
                                    AcceptType =
                                        dr.IsDBNull(dr.GetOrdinal("AcceptType"))
                                            ? Model.EnumType.PersonalCenterStructure.AcceptType.所有
                                            : (Model.EnumType.PersonalCenterStructure.AcceptType)
            ...
            int companyId,string zxsId)
        {
            ...
            string fields = " Id, CompanyId, Title, OperatorId,IssueTime,[Content],[Views],Files,IsDelete,(select ContactName from tbl_CompanyUser where tbl_CompanyUser.Id = tbl_News.OperatorId) as OperatorName,ZxsId ";

            var cmdQuery = new StringBuilder(" IsDelete = '0' ");
            cmdQuery.AppendFormat(" and CompanyId = {0} ", companyId);
            if (!string.IsNullOrEmpty(zxsId))
                cmdQuery.AppendFormat(" AND ZxsId='{0}' ", zxsId);
                ...
                , cmdQuery.ToString(), orderByString, string.Empty))
            {
                while (rdr.Read())
                {
                    ...
                    newInfo.ZxsId = rdr["ZxsId"].ToString();
                    totals.Add(newInfo);
                }
            }
        ...
        /// <param name="zxsId">专线商编号</param>
        /// <returns></returns>
        public IList<Model.PersonalCenterStructure.NoticeNews> GetAcceptNews(
            int pageSize,
            int pageIndex,
            ref int recordCount,
            int userId, int companyId,string zxsId)
        {
            if (pageSize <= 0 || pageIndex <= 0 || userId <= 0 || companyId <= 0) return null;

            IList<Model.PersonalCenterStructure.NoticeNews> lsNews = new List<Model.PersonalCenterStructure.NoticeNews>();

            string tableName = " SELECT a.ID, a.CompanyId, a.Title, a.Views, a.OperatorId, a.IssueTime, a.IsDelete,b.ContactName as OperatorName,a.ZxsId FROM tbl_News AS a LEFT JOIN tbl_CompanyUser AS b on a.OperatorId = b.Id ";
            string orderByString = "IssueTime DESC";
            string fields = " ID, Title,Views,OperatorId,IssueTime,OperatorName,ZxsId ";
            ...
            cmdQuery.AppendFormat(
                ...
                (int)Model.EnumType.PersonalCenterStructure.AcceptType.所有,
                ...
                , cmdQuery.ToString(), orderByString, string.Empty))
            {
                while (rdr.Read())
                {
                    var newInfo = new Model.PersonalCenterStructure.NoticeNews
                    {
                        Id = rdr.GetInt32(rdr.GetOrdinal("ID")),
                        Title = rdr.IsDBNull(rdr.GetOrdinal("Title")) ? "" : rdr.GetString(rdr.GetOrdinal("Title")),
        ...
        /// <param name="zxsId">专线商编号</param>
        /// <returns></returns>
        public bool IsNews(int companyId, int departId, int UserId,string zxsId)
        {
            StringBuilder query = new StringBuilder();
            query.AppendFormat(
            ...
            query.AppendFormat(" AND ZxsId='{0}' ", zxsId);
