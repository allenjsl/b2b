...
namespace EyouSoft.DAL.CompanyStructure
{
    ...
    public class SysHandleLogs : Toolkit.DAL.DALBase, IDAL.CompanyStructure.ISysHandleLogs
    {
        ...
        private const string SqlInsertSysHandleLogs = "insert into tbl_SysHandleLogs (ID,OperatorId,DepatId,CompanyId,ModuleId,EventCode,EventMessage,EventTitle,EventIp,ZxsId) values (@ID,@OperatorId,@DepatId,@CompanyId,@ModuleId,@EventCode,@EventMessage,@EventTitle,@EventIp,@ZxsId) ;";
        private const string SqlSelectSysHandleLogs = " SELECT a.ID, a.OperatorId, a.DepatId, a.CompanyId, a.ModuleId, a.EventCode, a.EventMessage, a.EventTitle, a.EventTime, a.EventIp, ISNULL(b.ContactName, '') AS UserName, ISNULL(c.DepartName, '') AS DepartName,a.ZxsId FROM         tbl_SysHandleLogs AS a LEFT OUTER JOIN tbl_CompanyUser AS b ON a.OperatorId = b.Id LEFT OUTER JOIN dbo.tbl_CompanyDepartment AS c ON a.DepatId = c.Id ";
        ...
        public bool Add(Model.CompanyStructure.SysHandleLogs model)
        {
            ...
            this._db.AddInParameter(cmd, "ZxsId", DbType.AnsiStringFixedLength, model.ZxsId);
            ...
            Model.CompanyStructure.QueryHandleLog model)
        {
            ...
            string fields = " Id, OperatorId, DepatId, CompanyId, ModuleId,EventCode,EventMessage,EventTitle,EventTime,EventIp,UserName,DepartName,ZxsId";
                ...
                , fields, cmdQuery.ToString(), orderByString, string.Empty))
            {
                while (rdr.Read())
                {
                    ...
                    sysHandleLogsInfo.ZxsId = rdr["ZxsId"].ToString();
