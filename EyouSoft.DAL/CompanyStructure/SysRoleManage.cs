...
namespace EyouSoft.DAL.CompanyStructure
{
    ...
    public class SysRoleManage : DALBase, IDAL.CompanyStructure.ISysRoleManage
    {
        ...
        /// <returns></returns>
        public IList<Model.CompanyStructure.SysRoleManage> GetList(int pageSize, int pageIndex, ref int recordCount, int companyId,string zxsId)
        {
            ...
            string query = string.Format("[CompanyId]={0} AND IsDelete='0' AND ZxsId='{1}' ", companyId,zxsId);
        ...
        public bool Add(Model.CompanyStructure.SysRoleManage model)
        {
            ...
            strSql.Append(@" INSERT INTO [tbl_SysRoleManage]
                                ([RoleName],[RoleChilds],[CompanyId],[IsDelete],[ZxsId])
                             VALUES
                                (@RoleName,@RoleChilds,@CompanyId,@IsDelete,@ZxsId);
                            select @@identity ;");
            DbCommand dc = _db.GetSqlStringCommand(strSql.ToString());
        ...
        public bool Delete(int companyId, params int[] roleId)
        {
            ...
            var strSql = new StringBuilder();
            strSql.Append(" UPDATE tbl_SysRoleManage SET IsDelete = '1' WHERE CompanyId = @CompanyId AND [RoleName]<>'管理员' ");
            if (roleId.Length == 1)
            {
