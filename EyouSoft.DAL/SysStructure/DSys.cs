...
namespace EyouSoft.DAL.SysStructure
{
    ...
    public class DSys : EyouSoft.Toolkit.DAL.DALBase, EyouSoft.IDAL.SysStructure.ISys
    {
        ...
        const string SQL_SELECT_GetSysRoleId = "SELECT [Id] FROM [tbl_SysRoleManage] WHERE [CompanyId]=@CompanyId AND RoleName='管理员' AND ZxsId IN(SELECT ZxsId FROM [tbl_Pt_ZhuanXianShang] WHERE CompanyId=@CompanyId AND T1=1) ";
        ...
        public IList<EyouSoft.Model.SysStructure.MSysInfo> GetSyss(int pageSize, int pageIndex, ref int recordCount, EyouSoft.Model.SysStructure.MSysSearchInfo searchInfo)
        {
            ...
            fields.Append(" ,(SELECT TOP(1) A.Id AS UserId,A.UserName,A.[Password],A.MD5Password,A.ContactName,A.ContactTel,A.ContactMobile,A.OnlineStatus,A.ZxsId FROM tbl_CompanyUser AS A WHERE A.CompanyId=(SELECT B.Id FROM [tbl_CompanyInfo] AS B WHERE B.SystemId=tbl_Sys.SysId) AND A.IsAdmin = '1' AND A.IsDelete='0' AND A.ZxsId=(SELECT ZxsId FROM [tbl_Pt_ZhuanXianShang] AS B WHERE B.CompanyId=A.CompanyId AND B.T1=1) FOR XML RAW,ROOT('root'))  AS AdminXML ");
            fields.Append(" ,(SELECT A.ZxsId FROM [tbl_Pt_ZhuanXianShang] AS A WHERE A.CompanyId=(SELECT B.Id FROM [tbl_CompanyInfo] AS B WHERE B.SystemId=tbl_Sys.SysId) AND A.T1=1) AS ZxsId");
            ...
            using (IDataReader rdr = DbHelper.ExecuteReader1(_db, pageSize, pageIndex, ref recordCount, tableName, fields.ToString(), string.Empty, orderByString, "SUM(SysId),Sum(CompanyId)"))
            {
                while (rdr.Read())
                {
                    ...
                    {
                        item.Password = new Model.CompanyStructure.PassWord { NoEncryptPassword = "000000" };
                    }
        ...
        public EyouSoft.Model.SysStructure.MSysInfo GetSysInfo(int sysId)
        {
            ...
            strSQL.Append(" ,(SELECT A.ZxsId FROM [tbl_Pt_ZhuanXianShang] AS A WHERE A.CompanyId=(SELECT B.Id FROM [tbl_CompanyInfo] AS B WHERE B.SystemId=tbl_Sys.SysId) AND A.T1=1) AS ZxsId ");
