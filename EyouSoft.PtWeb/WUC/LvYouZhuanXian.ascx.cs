...
{
    public partial class LvYouZhuanXian : System.Web.UI.UserControl
    {
        #region attributes
        /// <summary>
        /// 显示站点数量
        /// </summary>
        protected int XianShiZhanDianShuaLiang = 6;

        /// <summary>
        /// 默认的专线类别编号
        /// </summary>
        public int? MoRenZxlbId { get; set; }
        #endregion
        ...
        void InitInfo()
        {
            ...
            foreach (var item in items)
            {
                ...
                if (item.Zxlbs != null && item.Zxlbs.Count > 0)
                {
                    ...
                    {
                        string _class1 = " ";
                        if (MoRenZxlbId.HasValue && item1.ZxlbId == MoRenZxlbId.Value) _class1 = " class=\"on\" ";

                        s1.AppendFormat("<li {3}><a href=\"/xianlu/?zdid={0}&zxlbid={1}\">{2}</a></li>", item.ZhanDianId, item1.ZxlbId, item1.MingCheng, _class1);
                    }
                    s1.AppendFormat("</ul></div>");
                }
                else
                {
                    s1.AppendFormat("<div class=\"tishi mt10\"><img src=\"/images/sorry.png\">&nbsp;抱歉，该站点下暂无专线信息，请选择其它站点，谢谢！</div>");
                }
