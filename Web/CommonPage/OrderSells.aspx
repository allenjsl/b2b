                    ...
                    <tbody>
                        <tr>
                            <td bgcolor="#C1E5F5" align="left" style="padding: 5px 8px;" class="alertboxTableT bumenbox">
                                <b><a href='javascript:void(0);' onclick="OrderSellsPage.SearchFun('','<%#Eval("Id") %>')">
                                    <%#Eval("DepartName")%></a> </b>：
                                <%#GetAllDepart(Eval("Id"))%>
                            </td>
                        </tr>
                    </tbody>
                </table>
                                        ...
                                        <ItemTemplate>
                                            <td align="left">
                                                <label>
                                                    <%if (EyouSoft.Common.Utils.GetQueryStringValue("sModel") == "1")
                                                      { %>
                                                    <input type="radio" name="contactID" value="<%#Eval("Id")%>" />
                                                    <%}
                                                      else
                                                      { %>
                                                    <input type="checkbox" name="contactID" value="<%#Eval("Id")%>" />
                                                    <%} %>
                                                    <span data-deptid='<%#Eval("DepartId") %>' data-deptname='<%#Eval("DepartName") %>'>
        ...
        var OrderSellsPage = {
            ...
            InitSetSelect: function() {
                if (OrderSellsPage.aid) {
                    ...
                    var oldValue, oldText, deptId, deptName;
                    ...
                    if (sellsData && sellsData.a.length > 0 && '<%=Request.QueryString["sModel"]%>' != '1') {
                        this.AppendCheckbox(sellsData.a, sellsData.b, sellsData.c, sellsData.d);
                        return false;
                    }
                    if (oldValue && oldText) {
            ...
            AppendCheckbox: function(a, b, c, d) {
                var str = "";
                var sellsData = { a: [], b: [], c: [], d: [] };
                        ...
                        if (a[i] != "") {
                            ...
                            if ('<%=Request.QueryString["sModel"]%>' != '1') {
                                if (c && d && c.length >= i && d.length >= i) {
                                    ...
                                    str += '<label><input checked="checked" type="checkbox" value="' + a[i] + '" name="contactID"><span data-deptname="' + d[i] + '" data-deptid="' + c[i] + '">' + b[i] + '</span>&nbsp;&nbsp;</label>';
                                } else {
                                    str += '<label><input checked="checked" type="checkbox" value="' + a[i] + '" name="contactID"><span data-deptname="" data-deptid="">' + b[i] + '</span>&nbsp;&nbsp;</label>';
                                    sellsData.c.push("");
                                    sellsData.d.push("");
                                }
                            }
            ...
            SearchFun: function(key, deptId) {
                var data = { id: '<%=Request.QueryString["id"] %>', iframeId: '<%=Request.QueryString["iframeId"]%>', pIframeId: '<%=Request.QueryString["pIframeId"]%>', callBackFun: '<%=Request.QueryString["callBackFun"] %>', sModel: '<%=Request.QueryString["sModel"]%>', userName: '', deptId: '' };
                data.userName = key;
                data.deptId = deptId;
            ...
            BtnBind: function() {
                $("#a_btn").click(function() {
                    ...
                    var data = { id: '<%=Request.QueryString["id"] %>', value: OrderSellsPage.selectValue, text: OrderSellsPage.selectTxt, hide: '<%=Request.QueryString["hide"] %>', show: '<%=Request.QueryString["show"] %>', deptID: OrderSellsPage.deptID, deptName: OrderSellsPage.deptName };
                    //根据父级是否为弹窗传值
            ...
            if ('<%=Request.QueryString["sModel"]%>' != '1') {
                $("input[name='contactID']").click(function() {
                    ...
                    if (this.checked) {
                        ...
                        OrderSellsPage.AppendCheckbox(a, b, c, d);
                        OrderSellsPage.parentWindow["sellsData"] = sellsData;
                    } else {
                        var _s = this;
