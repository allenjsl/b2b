...
namespace Web.CommonPage
{
    ...
    public partial class SelectSection : BackPage
    {
        ...
        protected string GetDepart(object o, int flag)
        {
            int deptId = (int)o;
            var BLL = new EyouSoft.BLL.CompanyStructure.Department();
            var lst = BLL.GetList(this.SiteUserInfo.CompanyId,deptId,SiteUserInfo.ZxsId);
            ...
            if (lst != null && lst.Count > 0)
            {
                ...
                for (int i = 0; i < lst.Count; i++)
                {
                    ...
                    sb.AppendFormat("<input id=\"select_{1}{2}\" type=\"checkbox\" value=\"{0}\" data-pid='{3}' name=\"Operator3\" /><input id=\"select_{1}{2}\" type=\"radio\" value=\"{0}\" name=\"Operator3\" data-pid='{3}' />", lst[i].Id, i, flag, lst[i].PrevDepartId);
