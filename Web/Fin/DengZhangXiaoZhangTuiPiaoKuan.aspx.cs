...
namespace Web.Fin
{
    ...
    /// 出纳登帐销账-退票款
    /// </summary>
    public partial class DengZhangXiaoZhangTuiPiaoKuan : EyouSoft.Common.Page.BackPage
    {
        #region attributes
        /// <summary>
        /// 销账权限
        ...
        EyouSoft.Model.FinStructure.MXiaoZhangChaXunInfo GetChaXunInfo()
        {
            ...
            info.TuiPiaoKuan_JiaoYiHao=Utils.GetQueryStringValue("txtJiaoYiHao");
            info.TuiPiaoKuan_GysJiaoYiHao = Utils.GetQueryStringValue("txtGysJiaoYiHao");
            info.TuiPiaoKuan_DingDanHao = string.Empty;
            info.TuiPiaoKuan_DaiLiShangName = Utils.GetQueryStringValue("txtDaiLiShangName");
        ...
        void InitRpt()
        {
            ...
            var items = new EyouSoft.BLL.FinStructure.BDengZhang().GetXiaoZhangTuiPiaoKuans(CurrentUserCompanyID, pageSize, pageIndex, ref recordCount, chaXun);
        ...
        void XiaoZhang()
        {
            ...
            string[] txtTuiPiaoId = Utils.GetFormValues("txtTuiPiaoId[]");
            string[] txtXiaoZhangJinE = Utils.GetFormValues("txtXiaoZhangJinE[]");

            if (txtTuiPiaoId == null || txtTuiPiaoId.Length == 0 || txtXiaoZhangJinE == null || txtTuiPiaoId.Length != txtXiaoZhangJinE.Length)
                RCWE(UtilsCommons.AjaxReturnJson("0", "操作失败：表单异常。"));

            var items = new List<EyouSoft.Model.FinStructure.MXiaoZhang>();

            for (int i = 0; i < txtTuiPiaoId.Length; i++)
            {
                var item = new EyouSoft.Model.FinStructure.MXiaoZhang();
                item.OrderId = txtTuiPiaoId[i];
            ...
            int bllRetCode = new EyouSoft.BLL.FinStructure.BDengZhang().XiaoZhang(DengZhangId, SiteUserInfo.UserId, EyouSoft.Model.EnumType.FinStructure.XiaoZhangLeiXing1.销退票款, items);
