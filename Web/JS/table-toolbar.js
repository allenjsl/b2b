var tableToolbar = {
        ...
        if (this.deleteB.length > 0) {
            this.deleteB.click(function() {
                ...
                if (isSucess) {
                    ...
                    var _html = '<div style="padding: 10px 0 20px 10px;cursor: default;"><h1 style="margin:10px 5px;font-size:14px;font-weight:bold;"><img src="/images/y-tanhao.gif" style=" vertical-align:middle; margin-right:5px;" />  {{msg}}</h1><input type="button" onclick="$.unblockUI();return false;"  value="取消" " style="width:64px; height:24px; border:0 none; background:url(/images/cx.gif);" /> <input type="button" id="BLOCKUI_YES" value="确 定" style="width:64px; height:24px; border:0 none; margin-left:20px; background:url(/images/cx.gif);" /> </div>';
                    $.blockUI({
    ...
    ShowConfirmMsg: function(msg, callbackfun, callbackfun2) {
        var confirmMsg = msg;
        var _html = '<div style="padding: 10px 0 20px 10px;cursor: default;"><h1 style="margin:10px 5px;font-size:14px;font-weight:bold;"><img src="/images/y-tanhao.gif" style=" vertical-align:middle; margin-right:5px;" />  {{msg}}</h1><input type="button" id="BLOCKUI_NO" value="取消" " style="width:64px; height:24px; border:0 none; background:url(/images/cx.gif);" /> <input type="button" id="BLOCKUI_YES" value="确 定" style="width:64px; height:24px; border:0 none; margin-left:20px; background:url(/images/cx.gif);" /> </div>';
        $.blockUI({
            message: _html.replace(/{{msg}}/, confirmMsg),
            css: { backgroundColor: "#E9F4F9", borderColor: "#00446b", borderWidth: '1px', cursor: "pointer", color: "#ed0000", width: '375px' }
        });
    ...
    _dialogHtml: '<h3 style="padding:20px 0;font-weight:bold">{{msg}}</h3>',
    _initTableStyle: function() {
        //隔行,滑动,点击 变色.+ 单选框选中的行 变色:
        //this.container.find("tr:even").addClass('odd');
        //this.container.find("tr:odd").addClass("even");
    ...
    */
        ...
        update_msg2: "只能选择一{{colName}} 进行修改!",
...
(function($) {
    $.fn.autoAdd = function(options) {
        ...
        if (!settings.isEnable) {
            content.find("tr[data-isedit='1']").each(function() {
                $(this).find("input,textarea").attr("readonly", "readonly").css({ "background-color": "#dadada" });
                $(this).find("input[data-class='datetime']").attr("onfocus", "");
                $(this).find("." + settings.delButtonClass).hide();
        ...
        settings.changeInput.change(function() {
            ...
            if (ival > 0) {
                if (ival > settings.max || ival < settings.min) {
                    ...
                    addRow($(this).val(), null, null, settings.isEnable);
                }
            } else {
        ...
        function addRow(num, isInsert, target, isEnable) {
            //console.log(temp);
            //var strTemp = "";
            for (var i = 0; i < num - count; i++) {
                ...
                if (isEnable) {
                    ...
                    temp.find("input,textarea,select,:checkbox").attr("id", "").removeAttr("disabled").removeAttr("readonly").css("background-color", "#FFFFFF");
                    temp.find("input[data-class='datetime']").bind("click", function() { WdatePicker() });
                    temp.find("." + settings.delButtonClass).show();
                }
                ...
                if (temp.find(".edui-default").length > 0) {
                    temp.find(".edui-default").remove();
                }
        ...
        content.find("." + settings.moveUpClass).bind("click", function() {
            ...
            if (tr.find(".edui-default").length) {
                enowUEditor.destroy(tr.find(".edui-default").attr("id"));
            }
        ...
        function delRow(row) {
            ...
            //间隔行处理
            var _$next = row.next();
            if (_$next.attr("i_jiange") == "1") _$next.remove();
...
/*
var pcToobar = {
    _goptions: [], _isg: false,
    //多个联动菜单初始化
    ginit: function(options) {
        this._goptions.push(options);
        this.next();
    },
    next: function() {
        if (!this._isg && this._goptions.length > 0) {
            this._isg = true;
            var __options = this._goptions.shift();
            this.init(__options);
        }
    },
    init: function(options) {
        ...
        if (obj_g) {
            obj_g.html("").append(_self.defaultShow);
            pcToobar._post("g", obj_g, { "get": "g" }, function() { obj_g.change(); _self.next(); });
            obj_g.change(function() {
                if (obj_p && obj_g.val()) {
                    obj_p.append(_self.loadShow);
                    pcToobar._post("p", obj_p, { "get": "p", "gid": obj_g.val() }, function() { obj_p.change(); _self.next(); });
                } else _self.next();
            })
        }
        if (obj_p) {
            obj_p.html("").append(_self.defaultShow);
            obj_p.change(function() {
                if (obj_c && obj_p.val()) {
                    obj_c.append(_self.loadShow);
                    pcToobar._post("c", obj_c, { "get": "c", "pid": obj_p.val() }, function() { obj_c.change(); _self.next(); });
                } else _self.next();
            })
            if (obj_g == null) {
                pcToobar._post("p", obj_p, { "get": "p", "gid": "1" }, function() { obj_p.change(); _self.next(); });
            }
        }
        if (obj_c) {
            obj_c.html("").append(_self.defaultShow);
            obj_c.change(function() {
                if (obj_x && obj_c.val()) {
                    obj_x.append(_self.loadShow);
                    pcToobar._post("x", obj_x, { "get": "x", "cid": obj_c.val() }, function() { _self.next(); });
                } else _self.next();
            })
            if (obj_p == null) {
                pcToobar._post("c", obj_c, { "get": "c", "pid": "1" }, function() { obj_c.change(); _self.next(); });
            }
        }
        if (obj_x) {
    ...
    _options: { gID: "", pID: "", cID: "", xID: "", gSelect: "", pSelect: "", cSelect: "", xSelect: "", comID: "", isCy: "0" },
    ...
    _post: function(gt, o, para, fun) {
        ...
        $.ajax({
            type: "get",
            cache: false,
            url: "/ashx/GetProvinceAndCity.ashx?companyId=" + _self._options.comID + "&isCy=" + _self._options.isCy + "&" + $.param(para),
            dataType: "json",
...
var gscx = {
    ...
    _post: function(t, options, pv, fn) {
        ...
        $.ajax({
            ...
            url: "/ashx/GetProvinceAndCity.ashx?companyId=" + options.comid + "&isCy=" + options.t + "&" + $.param(_data),
    ...
    //options={gid:"#国家下拉菜单",sid:"#省份下拉菜单",cid:"#城市下拉菜单",xid:"#县区下拉菜单",gv:"国家选中的值",sv:"省份选中的值",cv:"城市选中的值",xv:"县区选中的值",comid:"公司编号",t:"0所有城市 1常用城市"}
    init: function(options) {
        var _options = { gid: "", sid: "", cid: "", xid: "", gv: "", sv: "", cv: "", xv: "", comid: "", t: "0" };
...
var pcToobar = {
    ...
    init: function(options) {
        var _options = { gid: options.gID, sid: options.pID, cid: options.cID, xid: options.xID, gv: options.gSelect, sv: options.pSelect, cv: options.cSelect, xv: options.xSelect, comid: options.comID, t: options.isCy }
        gscx.init(_options);
    }
};
...
var toXls = {
    ...
    __toXls: function(options) {
        //if (typeof (pagingRecordCount) == "undefined") pagingRecordCount = 0;
        if (!window.pagingRecordCount) pagingRecordCount = 0;

        //无数据导出时是否提示
        if (options.isEmptyMessage) {
    ...
    //int options:{"selector":"导出按钮选择器","url":"导出请求页面URL，默认当前URL","isEmptyMessage":"无数据导出时是否提示"}
    init: function(options) {
        options = options || {};
        options = $.extend({ "selector": "", "url": window.location.pathname, isEmptyMessage: true }, options);
...
//toxls 导出列表 url:导出请求页面URL，默认当前URL  无数据时提示无数据
function toXls1(url) {
    url = url || window.location.pathname;
    return toXls.__toXls({ "url": url, isEmptyMessage: true });
}

//toxls 导出列表 url:导出请求页面URL，默认当前URL 无数据时不提示无数据
function toXls2(url) {
    url = url || window.location.pathname;
    return toXls.__toXls({ "url": url, isEmptyMessage: false });
}
...
var BtFun = {
    InitBindBt: function(tdContainer) {
        $("#liststyle").find("td[data-class='" + tdContainer + "']").mouseover(function() {
            var _s = $(this);
            _s.unbind("mouseover");
            var tourID = _s.attr("data-tourId");
            if (_s.find("b[class!='fontgray']").length > 0) {
                $.ajax({
                    type: "get",
                    url: '/Ashx/GetSourcesByType.ashx?tourId=' + tourID,
                    dataType: "json",
                    success: function(JsonData) {
                        if (JsonData.list.length > 0) {
                            for (var i = 0; i < JsonData.list.length; i++) {
                                _s.find("div[data-type='" + JsonData.list[i].type + "']").append(JsonData.list[i].sourceNamelist.split(',').join("<br />"));
                                if ($.trim(_s.find("div[data-type='" + JsonData.list[i].type + "']").html()) != "") {
                                    _s.find("div[data-type='" + JsonData.list[i].type + "']").append(" <br />")
                                }
                            }
                            _s.find("b[class!='fontgray']").each(function() {
                                var _ts = $(this);
...
var wuc = {
    ...
    //客户选择
    keHuXuanZe: function(options) {
        var _url = "/commonpage/kehuxuanze.aspx?";
        top.Boxy.iframeDialog({ iframeUrl: _url + $.param(options), title: "选择客户单位", modal: true, width: "820", height: "378" });
    },
    //客户选择callback
    keHuXuanZe_callback: function(options) {
        var _data = options.data;
        $("#" + options.KeHuIdClientId).val(_data.keHuId);
        $("#" + options.KeHuMingChengClientId).val(_data.keHuName);

        this.keHuXuanZe_SetDuiFangCaoZuoRen(options);

        if (options.CallbackFn.length > 0) options.CallbackFn(_data);
    },
    //客户选择callback 设置对方操作人
    keHuXuanZe_SetDuiFangCaoZuoRen: function(options) {
        if (options.DuiFangCaoZuoRenClientId.length == 0) return;
        var _$obj = $("#" + options.DuiFangCaoZuoRenClientId);
        if (_$obj.length == 0) return;

        this.keHuZuanZe_InitDuiFangCaoZuoRenOption(options.DuiFangCaoZuoRenClientId, options.data.lxrs);
    },
    //客户选择 初始化对方操作人下拉菜单
    keHuZuanZe_InitDuiFangCaoZuoRenOption: function(duiFangCaoZuoRenClientId, data) {
        if (duiFangCaoZuoRenClientId.length == 0) return;
        var _$obj = $("#" + duiFangCaoZuoRenClientId);
        _$obj.empty();
        _$obj.append('<option value="">--请选择--</option>');
        ...
        for (var i = 0; i < data.length; i++) {
            _s.push('<option value="' + data[i].ContactId + '">' + data[i].Name + '</option>');
        }

        _$obj.append(_s.join(""));
    },
    //客户选择 初始化对方操作人
    keHuXuanZe_InitDuiFangCaoZuoRen: function(duiFangCaoZuoRenClientId, keHuIdClientId) {
        if (duiFangCaoZuoRenClientId.length == 0) return;
        if (keHuIdClientId.length == 0) return;
        var _keHuId = $("#" + keHuIdClientId).val();
        if (_keHuId.length == 0) return;
        ...
        /*缓存联系人信息*/
        /*_lxrs = _$obj.data("data-" + _keHuId);
        if (typeof _lxrs == "undefined") {
            $.ajax({
                type: "get", url: "/ashx/Handler.ashx?doType=getkehucaozuoren&txtkehuid=" + _keHuId, cache: true, dataType: "json", async: false,
                success: function(response) {
                    _lxrs = response;
                }
            });

            _$obj.data("data-" + _keHuId, _lxrs);
        }*/

        /*不缓存联系人信息*/
        $.ajax({
            type: "get", url: "/ashx/Handler.ashx?doType=getkehucaozuoren&txtkehuid=" + _keHuId, cache: false, dataType: "json", async: false,
            success: function(response) {
                _lxrs = response;
            }
        });

        this.keHuZuanZe_InitDuiFangCaoZuoRenOption(duiFangCaoZuoRenClientId, _lxrs);
        var _data_v = $.trim(_$obj.attr("data-v"));
        if (typeof _data_v != "undefined" && _data_v != "0" && _data_v.length > 0) _$obj.val(_$obj.attr("data-v"));
    }
};
//英文转中文
...
var enowUEditor = {
    ...
    //初始化编辑器 返回编辑器实例
    init: function(id, options) {
        var _op = { toolbars: EnowUeditor.toolbars1 };
        if (typeof options != "undefined") _op = options;
        var _ue = UE.getEditor(id, _op);
        this.cache.push({ id: id, ue: _ue });
        return _ue;
    },
    //销毁编辑器实例，使用textarea代替
    destroy: function(id) {
        var _ue = this.getEditor(id);
        if (_ue == null) return;
        this.sync(id);
        _ue.destroy();

        for (var i = this.cache.length - 1; i >= 0; i--) {
            if (this.cache[i].id == id) { this.cache.splice(i, 1); break; }
        }
    },
    //同步编辑器
    sync: function(id) {
        if (typeof id == "undefined") {
            var _ues = this.getEditors();
            if (_ues == null) return;

            for (var i = 0; i < _ues.lenght; i++) _ues[i].ue.sync();
        }
    ...
    getEditor: function(id) {
        ...
        for (var i = 0; i < this.cache.length; i++) {
            if (this.cache[i].id == id) _ue = this.cache[i].ue;
        }
        return _ue;
    },
    //获取所有编辑器信息
