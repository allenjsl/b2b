    ...
    public partial class CityAdd : EyouSoft.Common.Page.BackPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (method == "")
            {
                ...
                var proList = new EyouSoft.BLL.CompanyStructure.City().GetShengFens(CurrentUserCompanyID, chaXun);
                if (proList != null && proList.Count > 0)
                {
                    selProvince.DataTextField = "ShengFenName";
                    selProvince.DataValueField = "ShengFenId";
                ...
                if (cId != 0)//初始化城市信息
                {
                    ...
                    if (cityModel != null)
                    {
                        ...
                        this.lit_option.Text = UtilsCommons.GetEnumDDL(EyouSoft.Common.EnumObj.GetList(typeof(EyouSoft.Model.EnumType.CompanyStructure.ChengShiDiQu)), ((int)cityModel.DiQu).ToString());
                    }
                    return;
                }
                else
                {
                    this.lit_option.Text = UtilsCommons.GetEnumDDL(EyouSoft.Common.EnumObj.GetList(typeof(EyouSoft.Model.EnumType.CompanyStructure.ChengShiDiQu)), "0");
                }
                #endregion
            }
            else
            {
                ...
                cityModel.DiQu = (EyouSoft.Model.EnumType.CompanyStructure.ChengShiDiQu)Utils.GetInt(Utils.GetFormValue("ddlArea"));
                if (cId != 0)
                {  //修改城市
                    if (cityModel != null)
                    {
