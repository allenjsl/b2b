<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FileAdd.aspx.cs" Inherits="Web.UserCenter.FileAdd"
        ...
        var File = {
            Save: function() {
                if (File.Validate()) {
                    ...
                    $.newAjax({
                        type: "post",
                        url: "/UserCenter/FileAdd.aspx?" + $.param(Data),
            ...
            var _selfs = $("#<%=btnSave.ClientID %>");
                ...
                if (fileName == "") {
                    tableToolbar._showMsg("文档名称不能为空!");
                    return false;
                }
                var _filea = $("input[name='<%=UploadControl1.ClientHideID %>']").length;
                var _fileb = $("input[name='<%=UploadControl1.YuanFilePathClientName %>']").length;
                if (_filea == 0 && _fileb==0) {
                    tableToolbar._showMsg("附件不能为空!");
                    return false;
                }
