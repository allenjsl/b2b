...
<span id="span<%=this.SetPriv %>">
    ...
    <%} %>
    <span data-class="hideDeptInfo" data-deptid="<%=this.ClientDeptID %>" data-deptname="<%=this.ClientDeptName %>">
    </span></span>
...
<script type="text/javascript">
    $(function() {
        ...
        if ('<%=this.IsNotValid.ToString().ToLower() %>' == "false") {
            $("#<%=this.SellsNameClient %>").removeAttr("valid");
            $("#<%=this.SellsIDClient %>").removeAttr("valid");
        }
    })  
</script>
