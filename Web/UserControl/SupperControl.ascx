...
<a id="<%=btnID %>" class="Offers xuanyong" href="javascript:void(0);"></a>

<script type="text/javascript">
    $(function() {
        $("#<%=btnID %>,#<%=ClientText %>").live("click", function() {
            var url = "/CommonPage/UserSupper.aspx?";
            var hideObj = $("#<%=ClientValue %>");
            var showObj = $("#<%=ClientValue %>").attr("id");
            var type = "<%=SupplierType %>";
            url += $.param({ suppliertype: type, callBack: "<%=CallBack %>", hideID:$("#"+showObj).val(),pIframeID: "<%=IframeID %>", isall: "<%=AllType %>" })
