<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UploadControl.ascx.cs"
    Inherits="Web.UserControl.UploadControl" %>
...
<div style="display:none" id="<%=ClientID %>_DEL_DIV"></div>
<script type="text/javascript">
    function delUploadYuanFile(obj) {
        var _$li = $(obj).closest("li");
        var _delFileId = _$li.find("input[name='<%=YuanFileIdClientName %>']").val();        
        var _$ul = _$li.parent();
        _$li.remove();
        if (_$ul.find("li").length == 0) _$ul.remove();

        $("#<%=DelYuanFileIdDivClientName %>").append('<input type="hidden" value="' + _delFileId + '" name="<%=DelYuanFileIdClientName %>">');
    }	  
</script>
<asp:Literal runat="server" ID="ltrTiShiXinXi"></asp:Literal>
<style type="text/css">
.upload_y_ul {list-style:none; float:left;margin-left:5px;height:40px; line-height:40px; width:320px;}
.upload_y_ul li{list-style:none;float:left; margin-left:8px; line-height:40px;}
.upload_y_ul li.file{}
.upload_y_ul li span{padding-left:2px;}
.upload_y_ul li img{ margin-top:-3px;}
</style>
